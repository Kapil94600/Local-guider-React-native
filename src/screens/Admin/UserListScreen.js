...
// 🔥 Get image URL (same as in ProfileUpdateScreen)
const getImageUrl = (path) => {
    if (!path) return null;
    if (path.startsWith("http")) return path;
    const filename = path.split("/").pop();   // extract only the filename
...
export default function UserListScreen({ navigation }) {
    ...
    const fetchUsers = async (page = 1, append = false) => {
        try {
            ...
            let totalPagesFromApi = responseData.totalPages || responseData.last_page || Math.ceil(total / PAGE_SIZE);

            // Handle nested data if present
            if (usersData && !Array.isArray(usersData) && usersData.users) {
                usersData = usersData.users;
            }

            // ✅ Filter: keep only tourists (photographer=false, guider=false)
            const touristUsers = usersData.filter(user => !user.photographer && !user.guider);

            // Format users with unique id fallback
            const formattedUsers = touristUsers.map((user, index) => ({
                id: user.id || user._id || `temp_${page}_${index}_${Date.now()}`,
                ...
                profilePicture: user.profile || user.profilePicture, // use 'profile' field if present
            ...
            if (append) {
                ...
                setUsers(prev => {
                    ...
                    const newUsers = formattedUsers.filter(u => !existingIds.has(u.id));
    ...
    // ✅ Update user (send as JSON with userId)
    const updateUser = async () => {
        ...
        try {
            ...
            const payload = {
                userId: selectedUser.id,  // ✅ use 'userId'
                name: editName,
                email: editEmail,
                phone: editPhone,
                ...
                gender: editGender,
                dob: editDob,
                isActive: editStatus === "active" ? "true" : "false",
            };
            // Remove empty fields to avoid sending nulls (optional)
            Object.keys(payload).forEach(key => payload[key] === undefined && delete payload[key]);

            const response = await api.post(API.UPDATE_PROFILE, payload);
    ...
    // ✅ Delete user (send as JSON with userId)
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            setActionLoading(true);
                            const response = await api.post(API.DELETE_USER, { userId: userId }); // ✅ send JSON
                            if (response.data?.status) {
    ...
    // ✅ Update user status (send as JSON with userId and isActive)
    ...
    const renderFilterButton = (filterType, label, icon) => (
        ...
        >
            <Ionicons name={icon} size={16} color={filter === filterType ? "#2c5a73" : "#fff"} />
    ...
    // Individual user item (with image error handling)
    const UserItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={styles.card} onPress={() => showUserDetails(item)} activeOpacity={0.9}>
                <LinearGradient colors={['#ffffff', '#f8fafc']} style={styles.cardGradient}>
                    <View style={styles.cardHeader}>
                        ...
                        <View style={styles.userInfo}>
                            ...
                            <View style={styles.userMetaRow}>
                                ...
                                <Text style={styles.userPhone} numberOfLines={1}>{item.countryCode} {item.phone}</Text>
                            </View>
                        </View>
                    ...
                    <View style={styles.cardFooter}>
                        ...
                        <View style={[styles.roleBadge, { backgroundColor: "#3B82F620" }]}>
                            <Text style={[styles.roleText, { color: "#3B82F6" }]}>Tourist</Text>
                        </View>
                    </View>
                </LinearGradient>
            </TouchableOpacity>
        ...
        <SafeAreaView style={styles.container}>
            ...
            <LinearGradient colors={['#2c5a73', '#1e3c4f']} style={styles.header}>
                <View style={styles.headerTop}>
                    ...
                    <View style={styles.headerCenter}>
                        ...
                        <Text style={styles.subtitle}>Tourist Management</Text>
                    </View>
                    <View style={styles.headerRight}>
                        <TouchableOpacity style={styles.downloadButton} onPress={downloadUsers} disabled={downloading}>
                        ...
                        <View style={styles.userCountContainer}>
                            ...
                            <Text style={styles.userCountLabel}>Tourists</Text>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.searchContainer}>
                ...
                <TextInput
                    ...
                    placeholder="Search tourists by name, email or phone..."
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    refreshControl={
                        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#2c5a73"]} tintColor="#2c5a73" />
                    }
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptySubtitle}>
                                {searchQuery ? "Try a different search" : "No tourist users registered yet"}
                            </Text>
                        </View>
                    }
                />
                        ...
                        {selectedUser && (
                            <ScrollView showsVerticalScrollIndicator={false}>
                                <View style={styles.modalProfileSection}>
                                    <View style={[styles.modalAvatar, { backgroundColor: getAvatarColor(selectedUser.name) }]}>
                                    ...
                                    <View style={styles.modalUserMeta}>
                                        <View style={[styles.roleBadge, { backgroundColor: "#3B82F620", paddingHorizontal: 12, paddingVertical: 4 }]}>
                                            <Text style={[styles.roleText, { color: "#3B82F6" }]}>Tourist</Text>
                                        </View>
                                        <View style={[styles.statusBadge, { backgroundColor: selectedUser.status === "active" ? "#10B981" : "#EF4444" }]}>
                                            <Ionicons name={selectedUser.status === "active" ? "checkmark-circle" : "close-circle"} size={14} color="#fff" />
                                ...
                                <View style={styles.modalSection}>
                                    ...
                                    <View style={styles.modalInfoRow}>
                                        ...
                                        <Text style={[styles.modalInfoValue, { color: "#10B981", fontWeight: "bold" }]}>₹{selectedUser.balance?.toLocaleString() || 0}</Text>
                                    </View>
                                    <View style={styles.modalInfoRow}>
                                ...
                                <View style={styles.modalActions}>
                                    <TouchableOpacity style={[styles.actionButton, styles.editButton]} onPress={() => { setDetailsModalVisible(false); openEditModal(selectedUser); }}>
                                        <Ionicons name="create-outline" size={18} color="#fff" />
                                        <Text style={styles.actionButtonText}>Edit</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.actionButton, selectedUser.status === "active" ? styles.deactivateButton : styles.activateButton]} onPress={() => updateUserStatus(selectedUser.id, selectedUser.status === "active", selectedUser.name)}>
                                        <Ionicons name={selectedUser.status === "active" ? "pause-circle" : "play-circle"} size={18} color="#fff" />
                                        <Text style={styles.actionButtonText}>{selectedUser.status === "active" ? "Deactivate" : "Activate"}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.actionButton, styles.deleteButton]} onPress={() => deleteUser(selectedUser.id, selectedUser.name)}>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <View style={styles.editForm}>
                                ...
                                <TextInput style={styles.input} value={editName} onChangeText={setEditName} placeholder="Enter name" placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Email</Text>
                                <TextInput style={styles.input} value={editEmail} onChangeText={setEditEmail} placeholder="Enter email" keyboardType="email-address" autoCapitalize="none" placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Phone</Text>
                                <TextInput style={styles.input} value={editPhone} onChangeText={setEditPhone} placeholder="Enter phone" keyboardType="phone-pad" placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Username</Text>
                                <TextInput style={styles.input} value={editUsername} onChangeText={setEditUsername} placeholder="Enter username" autoCapitalize="none" placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Address</Text>
                                <TextInput style={[styles.input, styles.textArea]} value={editAddress} onChangeText={setEditAddress} placeholder="Enter address" multiline numberOfLines={2} placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Gender</Text>
                                <View style={styles.genderContainer}>
                                    <TouchableOpacity style={[styles.genderOption, editGender === "male" && styles.genderOptionSelected]} onPress={() => setEditGender("male")}>
                                        <Text style={[styles.genderText, editGender === "male" && styles.genderTextSelected]}>Male</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.genderOption, editGender === "female" && styles.genderOptionSelected]} onPress={() => setEditGender("female")}>
                                        <Text style={[styles.genderText, editGender === "female" && styles.genderTextSelected]}>Female</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.genderOption, editGender === "other" && styles.genderOptionSelected]} onPress={() => setEditGender("other")}>
                                        <Text style={[styles.genderText, editGender === "other" && styles.genderTextSelected]}>Other</Text>
                                    </TouchableOpacity>
                                </View>
                                <Text style={styles.inputLabel}>Date of Birth</Text>
                                <TextInput style={styles.input} value={editDob} onChangeText={setEditDob} placeholder="YYYY-MM-DD" placeholderTextColor="#94a3b8" />
                                <Text style={styles.inputLabel}>Status</Text>
                                <View style={styles.statusContainer}>
                                    <TouchableOpacity style={[styles.statusOption, editStatus === "active" && styles.statusOptionActive]} onPress={() => setEditStatus("active")}>
                                        <Text style={[styles.statusText, editStatus === "active" && styles.statusTextActive]}>Active</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.statusOption, editStatus === "inactive" && styles.statusOptionInactive]} onPress={() => setEditStatus("inactive")}>
                                        <Text style={[styles.statusText, editStatus === "inactive" && styles.statusTextInactive]}>Inactive</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                            <View style={styles.editActions}>
                                <TouchableOpacity style={[styles.editButton, styles.cancelEditButton]} onPress={() => setEditModalVisible(false)}>
                                    <Text style={styles.cancelEditText}>Cancel</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.editButton, styles.saveEditButton]} onPress={updateUser} disabled={actionLoading}>
                                    {actionLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.saveEditText}>Save Changes</Text>}
                                </TouchableOpacity>
                            </View>
                        </ScrollView>
                    </View>
...
const styles = StyleSheet.create({
    ...
    userCountContainer: { backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, alignItems: 'center', minWidth: 60 },
    userCountNumber: { fontSize: 18, fontWeight: 'bold', color: '#fff' },
    userCountLabel: { fontSize: 10, color: '#fff', opacity: 0.8 },
    filterScroll: { marginTop: 5 },
    filterContainer: { flexDirection: 'row', gap: 8 },
    filterButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.15)', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, gap: 6 },
    filterButtonActive: { backgroundColor: '#fff' },
    filterButtonText: { fontSize: 12, fontWeight: '600', color: '#fff' },
    filterButtonTextActive: { color: '#2c5a73' },
    ...
    avatar: { width: 50, height: 50, borderRadius: 25, alignItems: "center", justifyContent: "center", marginRight: 12, overflow: "hidden", elevation: 2 },
    avatarImage: { width: "100%", height: "100%" },
    avatarText: { fontSize: 22, fontWeight: "bold", color: "#fff" },
    userInfo: { flex: 1 },
    userName: { fontSize: 16, fontWeight: "bold", color: "#1e293b", marginBottom: 4 },
    userEmail: { fontSize: 12, color: "#64748b", marginBottom: 4 },
    userMetaRow: { flexDirection: "row", alignItems: "center", gap: 4 },
    userPhone: { fontSize: 12, color: "#64748b", marginLeft: 2 },
    statusDot: { width: 10, height: 10, borderRadius: 5, marginLeft: 8 },
    cardFooter: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: "#e2e8f0", paddingTop: 12 },
    statItem: { flexDirection: "row", alignItems: "center", gap: 4 },
    statValue: { fontSize: 12, color: "#2c5a73", fontWeight: "600" },
    statDivider: { width: 1, height: 20, backgroundColor: "#e2e8f0", marginHorizontal: 8 },
    roleBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, marginLeft: 4 },
    ...
    modalAvatar: { width: 90, height: 90, borderRadius: 45, alignItems: "center", justifyContent: "center", marginBottom: 12, overflow: "hidden", elevation: 4 },
    modalAvatarImage: { width: "100%", height: "100%" },
    modalAvatarText: { fontSize: 36, fontWeight: "bold", color: "#fff" },
    modalUserName: { fontSize: 22, fontWeight: "bold", color: "#1e293b", marginBottom: 8 },
    modalUserMeta: { flexDirection: "row", gap: 8 },
    ...
    modalInfoLabel: { fontSize: 14, color: "#64748b", marginLeft: 8, marginRight: 4, width: 90 },
    ...
    input: { borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 12, padding: 14, fontSize: 14, color: "#1e293b", backgroundColor: "#f8fafc" },
    textArea: { height: 80, textAlignVertical: "top" },
    genderContainer: { flexDirection: "row", gap: 12 },
    genderOption: { flex: 1, paddingVertical: 12, borderRadius: 12, alignItems: "center", borderWidth: 1, borderColor: "#e2e8f0", backgroundColor: "#f8fafc" },
    genderOptionSelected: { backgroundColor: "#2c5a73", borderColor: "#2c5a73" },
    genderText: { fontSize: 14, color: "#64748b", fontWeight: "500" },
    genderTextSelected: { color: "#fff" },
    statusContainer: { flexDirection: "row", gap: 12, marginBottom: 20 },
    statusOption: { flex: 1, paddingVertical: 12, borderRadius: 12, alignItems: "center", borderWidth: 1, borderColor: "#e2e8f0", backgroundColor: "#f8fafc" },
    statusOptionActive: { backgroundColor: "#10B981", borderColor: "#10B981" },
    statusOptionInactive: { backgroundColor: "#EF4444", borderColor: "#EF4444" },
    statusText: { fontSize: 14, color: "#64748b", fontWeight: "500" },
    statusTextActive: { color: "#fff" },
    statusTextInactive: { color: "#fff" },
    editActions: { flexDirection: "row", gap: 12, marginTop: 10 },
    editButton: { flex: 1, paddingVertical: 14, borderRadius: 12, alignItems: "center" },
    cancelEditButton: { backgroundColor: "#f1f5f9", borderWidth: 1, borderColor: "#e2e8f0" },
    saveEditButton: { backgroundColor: "#2c5a73" },
    cancelEditText: { color: "#64748b", fontSize: 15, fontWeight: "600" },
    saveEditText: { color: "#fff", fontSize: 15, fontWeight: "600" },
